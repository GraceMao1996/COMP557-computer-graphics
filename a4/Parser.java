...
public class Parser {
	...
	public static Render createRender(Node dataNode) {
		...
		if(jitterAttr !=null){//If there is a jitter attribute in the parser tag
			...
			render.jitter=s.nextBoolean();//Set jitter to the value defined in the xml attribute
			s.close();
		}
	...
	public static Quadric createQuadric(Node dataNode) {
		...
		Node a_a = dataNode.getAttributes().getNamedItem("a_a");
		if ( a_a != null ) {
			quadric.a_a = (float)Double.parseDouble( a_a.getNodeValue() );
		}
		Node b_b = dataNode.getAttributes().getNamedItem("b_b");
		if ( b_b != null ) {
			quadric.b_b = (float)Double.parseDouble( b_b.getNodeValue() );
