...
public class MeshSimplificationApp implements SceneGraphNode, Interactor {
    ...
    private void loadSoupBuildAndSubdivide( String filename ) {
        ...
        if ( heds.faces.size() > 4 ) {
        	...
        	currentHE = heds.selectEdge();
        }
        else currentHE = heds.faces.iterator().next().he;
    }
        ...
        component.addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_SPACE) {
                	...
                	if ( e.isShiftDown() ) {
                		do {
                			...
                			// if ( noMoreCollapse ) break;
                			  heds.setRegularizationWeight(regularizationWeight.getDefaultValue());
                      		if (currentHE==null) {
                      			System.out.println("no edge to collapse");
                      			return;
                      		}
                    		heds.EdgeCollapse(currentHE);
                    	    currentHE = heds.selectEdge();
                		} while ( true );
                	} else {
                	    heds.setRegularizationWeight(regularizationWeight.getDefaultValue());
                	...
                	heds.undoCollapse();
                	//currentHE = heds.redoListHalfEdge.getLast();
